           ...
           )
        {
            ...
            for (L = 1; L < order; L++)
            {
                ...
                for (m = L + 1; m <= order; m++)
                {
                    ...
                    {
                        VArr[L, m] = ((2 * L - 1) / (L - m)) * r[2] * temp * VArr[L - 1, m] - ((L + m - 1) / (L - m)) * temp * MathTimeLib.globals.re * VArr[L - 2, m];
                        WArr[L, m] = ((2 * L - 1) / (L - m)) * r[2] * temp * WArr[L - 1, m] - ((L + m - 1) / (L - m)) * temp * MathTimeLib.globals.re * WArr[L - 2, m];
                    }
                }
            }
