...
namespace AstroLibMethods
{
    ...
    {
        public string AstroLibVersion = "AstroLib Version 2019-10-23";
    ...
    public void azel_radec
        ...
        * ----------------------------------------------------------------------------*/
           ...
           )
        {
            ...
            // try out montenbruick approach
            double[,] LegArr1 = new double[order + 2, order + 2];
            ...
            for (L = 2; L <= order; L++)
            {
                ...
                for (m = 0; m <= L + 1; m++)
                {
                    ...
                    // alt approach
                    if (L == m)
                        LegArr1[L, m] = (2 * L - 1) * LegArr[1, 1] * LegArr[L - 1, m - 1];  // sectoral part
                    else
                    {
                        if (L == m + 1)
                            LegArr1[L, m] = (2 * m + 1) * LegArr[1, 0] * LegArr1[m, m];
                        else
                            LegArr1[L, m] = 1.0 / (L - m) * ((2 * L - 1) * LegArr[1, 1] * LegArr1[L - 1, m] - (L + m - 1) * LegArr1[L - 2, m]);
                    }
                }   // for m
            }   // for L
        ...
        * ----------------------------------------------------------------------------*/
        ...
        public void FullGeop
            ...
            )
            {
                ...
                LegPoly(latgc, lon, order, gravData, out trigArr, out LegArr);
