...
namespace AstroLibMethods
{
    ...
    {
        public string AstroLibVersion = "AstroLib Version 2020-06-17";
...
* --------------------------------------------------------------------------- */
        ...
        * ----------------------------------------------------------------------------*/
        ...
        public void rv2flt
            (double[] reci, double[] veci, double ttt, double jdut1, double lod, double xp, double yp, int terms, double ddpsi, double ddeps,
           out double lon, out double latgc, out double rtasc, out double decl, out double fpa, out double az, out double magr, out double magv
            )
        {
            ...
            eci_ecef(ref reci, ref veci, EOPSPWLibr.iau80arr, MathTimeLib.Edirection.eto,
                ttt, jdut1, lod, xp, yp, 2, ddpsi, ddeps, EOpt.e80, ref recef, ref vecef);
        ...
        *   for satellite operations, orders up to about 120 are valid. thsi routine uses 
        ...
        * ----------------------------------------------------------------------------*/
           ...
           )
        {
            ...
            VArr[0, 0] = MathTimeLib.globals.re / magr;
            ...
            for (L = 2; L <= order + 1; L++)
            {
                VArr[L, 0] = (2.0 * L - 1) / L * recef[2] * temp * VArr[L - 1, 0] -
                     (L - 1.0) / L * temp * MathTimeLib.globals.re * VArr[L - 2, 0];
                WArr[L, 0] = 0.0;
            }
            ...
            for (L = 1; L <= order + 1; L++)
            {
                ...
                for (int k = m + 1; k <= order + 1; k++)
                {
                    VArr[k, m] = ((2.0 * k - 1) / (k - m)) * recef[2] * temp * VArr[k - 1, m] -
                        (k + m - 1.0) / (k - m) * temp * MathTimeLib.globals.re * VArr[k - 2, m];
                    WArr[k, m] = ((2.0 * k - 1) / (k - m)) * recef[2] * temp * WArr[k - 1, m] -
                        (k + m - 1.0) / (k - m) * temp * MathTimeLib.globals.re * WArr[k - 2, m];
                }
            }
            ...
            if (normalized == 'y')
            {
                VArrN[0, 0] = MathTimeLib.globals.re / magr;
                WArrN[0, 0] = 0.0;
                for (L = 1; L <= order + 1; L++)
                {
                    for (m = 0; m <= L; m++)
        ...
        * ----------------------------------------------------------------------------*/
        ...
        )
        {
            ...
            if (show == 'y')
            {
                ...
                straccum = straccum + "trigarr 2 2 sin  " + trigArr[2, 0].ToString() + "  cos   " + trigArr[2, 1].ToString() + "  tan   " + trigArr[2, 2].ToString() + "\n";
            }

            // ----------Partial derivatives of disturbing potential ------- }
            double magr = MathTimeLibr.mag(recef);
            ror = MathTimeLib.globals.re / magr;
            dRdr = 0.0;
            dRdlat = 0.0;
            dRdlon = 0.0;
            temp = ror;
            ...
            muor = MathTimeLib.globals.mu / magr;
            dRdr = -muor / magr * dRdr;
            dRdlat = muor * dRdlat;
            dRdlon = muor * dRdlon;
            ...
            //double tmp = MathTimeLib.globals.mu / (Math.Pow(MathTimeLibr.mag(recef), 3));
        ...
        * ----------------------------------------------------------------------------*/
        ...
        )
        {
            ...
            temp = MathTimeLib.globals.mu / (MathTimeLib.globals.re * MathTimeLib.globals.re);
            temp = 1.0;
        ...
        * ----------------------------------------------------------------------------*/
        ...
        )
        {
            ...
            rho = MathTimeLib.globals.re * MathTimeLib.globals.re / r_sqr;
            x0 = MathTimeLib.globals.re * recef[0] / r_sqr;          // Normalized
            y0 = MathTimeLib.globals.re * recef[1] / r_sqr;          // coordinates
            z0 = MathTimeLib.globals.re * recef[2] / r_sqr;
            ...
            V[0, 0] = MathTimeLib.globals.re / Math.Sqrt(r_sqr);
            W[0, 0] = 0.0;
            V[1, 0] = z0 * V[0, 0];
            W[1, 0] = 0.0;
            for (n = 2; n <= order; n++)
        ...
        * ---------------------------------------------------------------------------- */
        ...
        {
            double p, a, n, ecc, incl, raan, argp, nu, mean, e0, e,
                rx, ry, rz, vx, vy, vz,
        ...
        * ---------------------------------------------------------------------------- */
            ...
            )
        {
            ...
            a = 1.0 / (2.0 / magr - magv * magv / MathTimeLib.globals.mum);  // m
            n = Math.Sqrt(MathTimeLib.globals.mum / (a * a * a));
        ...
        *    Vallado and Alfano 2015
            ...
            )
        {
            ...
            if (anomflt.Equals("latlon"))
            {
                ...
                eci_ecef(ref reci, ref veci, iau80arr, MathTimeLib.Edirection.eto,
                    ttt, jdut1, lod, xp, yp, terms, ddpsi, ddeps, opt, ref recef, ref vecef);
        ...
        * ---------------------------------------------------------------------------- */
            ...
            )
        {
            ...
            if (anomflt.Equals("latlon"))
            {
                ...
                eci_ecef(ref reci, ref veci, iau80arr, MathTimeLib.Edirection.efrom, ttt, jdut1, lod, xp, yp, terms, ddpsi, ddeps, opt, ref recef, ref vecef);
