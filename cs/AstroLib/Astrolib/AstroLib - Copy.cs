...
//
//    current :
//               2 mar 18  david vallado
//                           fixes to lambert battin for multi-rev
//    changes :
...
namespace AstroLibMethods
{
    public class AstroLib
    {
        ...
        // use a struct for all the eop data so it can be processed
 ...
 * ---------------------------------------------------------------------------- */
        ...
        * --------------------------------------------------------------------------- */
        ...
        //    de          - orbital energy (low, high)           'l','h'
        ...
        public void lambertumins
            (
            double[] r1, double[] r2, Int32 nrev, char de,
...
*    -----------------------------------------------------------------------------*/
        ...
        {
            double a, s, chord, magr1, magr2, cosdeltanu, sindeltanu;
            double[] rcrossr, nr1, nr2, nunit, v2t, tempvec = new double[3];
            double y1, y2, c, x1, x2, en, e1, e2, r, sme, k;
            int i;
            ...
            if (MathTimeLibr.mag(tempvec) < 0.001)  // 1 meter
            {
                c = Math.Sqrt(y1);
                x1 = x2 = r = 0.0;
            ...
            else
            {
                ...
                e1 = Math.Acos(c * (r * r + y1 - c * c) / (r * y1));
                if (x1 < 0.0)
                    e1 = 2.0 * Math.PI - e1;
                // check if acos is larger than 1
                double temp = c * (r * r + y2 - c * c) / (r * y2);
                if (Math.Abs(temp) > 1.0)
                    e2 = Math.Sign(temp) * Math.Acos(1.0);
                else
                    e2 = Math.Acos(c * (r * r + y2 - c * c) / (r * y2));
                if (x2 < 0.0)
                    e2 = 2.0 * Math.PI - e2;
                if (e2 < e1)
                    e2 = e2 + 2.0 * Math.PI;
                ...
                tmaxrp = (MathTimeLib.globals.mu * (2.0 * nrev * Math.PI + k)) / Math.Pow(c * c - r * r, 1.5) +
                    (MathTimeLib.globals.mu * c * sindeltanu) / (y1 * y2);
            }
            sme = (r * r - c * c) * 0.5;
        ...
        * [v1t, v2t] = lambhodograph(r1, v1, r2, p, a, ecc, dnu, dtsec)
        ...
        )
        {
            ...
            else
            {
                ...
                // depending on the MathTimeLibr.cross product, this will be normal or in plane,
        ...
        *    thompson      AAS GNC 2018
        ...
        public void lambertbattin
               ...
               double altpad, out double[] v1t, out double[] v2t, out char hitearth, out string errorsum, out string errorout
               )
        {
            ...
            // alt approach for high energy(long way, direct/retro, multi - nrev) case
            if ((de.Equals('h')) && (nrev > 0))  // old df.Equals('r')
            {
                h1 = 0.0;
                h2 = 0.0;
                b = 0.0;
                ...
                while ((Math.Abs(xn - x) >= small) && (loops <= 20))
                {
                    ...
                    // uncomment next if show to get all iterations, and also uncomment errorsum in show == 'n'
                    // do in both yh and yb
                    //     if (show == 'y')
                ...
                errorsum = errorstr + "high " + nrev.ToString().PadLeft(3) + "   " + dm + "  " + de + dtwait.ToString("0.0000000").PadLeft(12) + " " +
                        ...
                        " " + (dnu * rad).ToString("0.0000000").PadLeft(15) + " " + hitearth +
                        " " + y.ToString() + " " + b.ToString("0.0000000").PadLeft(15);
            }
            else
            {
                // standard processing
                ...
                if (loops < 30)
                {
                    ...
                    double rad = 180.0 / Math.PI;
                    lambhodograph(r1, r2, v1, p, ecc, dnu, dtsec, out v1t, out v2t);
                    errorsum = errorstr + "low " + nrev.ToString().PadLeft(3) + "   " + dm + "  " + de + dtwait.ToString("0.0000000").PadLeft(12) + " " +
                            dtsec.ToString("0.#######").PadLeft(15) + " " + x.ToString("0.#######").PadLeft(15) + " " +
                            v1t[0].ToString("0.0000000").PadLeft(15) + v1t[1].ToString("0.0000000").PadLeft(15) + v1t[2].ToString("0.0000000").PadLeft(15) +
                            v2t[0].ToString("0.0000000").PadLeft(15) + v2t[1].ToString("0.0000000").PadLeft(15) + v2t[2].ToString("0.0000000").PadLeft(15) +
                            " " + loops.ToString().PadLeft(4) + " " + rp.ToString("0.0000000").PadLeft(15) + " " + a.ToString("0.0000000").PadLeft(15) +
        ...
        *    vallado       2013, 492, alg 58, ex 7-5
        ...
        public void lambertuniv
               (
               double[] r1, double[] r2, double[] v1, char dm, char de, Int32 nrev, double dtwait, double dtsec, double[,] tbi,
               double altpad, out double[] v1t, out double[] v2t, out char hitearth, out string errorsum, out string errorout
               )
        {
            const double small = 0.0000001;
            ...
            {
                if (de == 'l')  // pick an initial position  //dm == 's
                ...
                while ((Math.Abs(dtnew - dtsec) >= small) && (loops < numiter) && (ynegktr <= 10))
                {
                    ...
                    if (ynegktr < 10)
                    {
                        ...
                        errorout = errorout + "\r\n" + loops.ToString().PadLeft(3) + "   " + y.ToString("0.0000000").PadLeft(12) + "  " +
                            ...
                            psitmp.ToString() + " " + psinew.ToString() + " " + estr;
     ...
     dtsec.ToString("0.#######").PadLeft(15) + " " + psinew.ToString("0.#######").PadLeft(15) +
     v1t[0].ToString("0.0000000").PadLeft(15) + v1t[1].ToString("0.0000000").PadLeft(15) + v1t[2].ToString("0.0000000").PadLeft(15) +
     v2t[0].ToString("0.0000000").PadLeft(15) + v2t[1].ToString("0.0000000").PadLeft(15) + v2t[2].ToString("0.0000000").PadLeft(15) +
     " " + loops.ToString().PadLeft(4) + " " + rp.ToString("0.0000000").PadLeft(15) + " " + a.ToString("0.0000000").PadLeft(15) +
        ...
        * ---------------------------------------------------------------------------- */
        ...
        public void covct2cl(double[,] cartcov, double[] cartstate, string anomclass, out double[,] classcov, out double[,] tm)
        {
            double p, a, n, ecc, incl, raan, argp, nu, mean, arglat, truelon, lonper,
                ...
                sign_anode, cos_raan, sign_w, cos_w, w_scale, r_dot_e, cos_nu, sign_nu, nu_scale, p0, p1, p3, p4, p5, p6, dMde, dMdnu, temp;
            double[] reci = new double[3];
        ...
        * ---------------------------------------------------------------------------- */
            ...
            )
        {
            double p, a, ecc, incl, raan, argp, nu, mean, e0, e,
                ...
                p11, p12, p13, p21, p22, p23, p31, p32, p33, p0, p1, p2, p3, p4, p5, p6, p8, p9, p10, dMdnu, dMde, temp;
